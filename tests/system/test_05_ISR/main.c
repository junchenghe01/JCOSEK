...
void Global_Timer_Handler(void) {
    *(volatile uint32_t *)(MPC_GLOB_BASE + 0x0C) = 0x1;
}
/*
29: // Private Timer
*/
void Private_Timer_Handler(void) {
    ...
    *(volatile uint32_t *)(MPC_PRIV_BASE + 0x0C) = 0x1;
}
/*
30: // Watchdog
*/
void Watchdog_Handler(void) {
    *(volatile uint32_t *)(MPC_PRIV_BASE + 0x2C) = 0x1;
}
/*
34: // RTC
*/
void RTC_Handler(void) {
    *(volatile uint32_t *)(SYS_RTC_BASE + 0x0C) = 0x1;
}
/*
48: // SP804
*/
void SP804_Handler(void) {
        // 检查 Timer 1 是否触发
    if (*(volatile uint32_t *)SP804_T1_MIS != 0) {
        // 1. 硬件清除 Timer 1 中断 (写入任意值)
        *(volatile uint32_t *)SP804_T1_INTCLR = 0x1;
    ...
    // 检查 Timer 2 是否触发
    if (*(volatile uint32_t *)SP804_T2_MIS != 0) {
        // 1. 硬件清除 Timer 2 中断
        *(volatile uint32_t *)SP804_T2_INTCLR = 0x1;
    ...
    // IncrementCounter(0); // 驱动 OS 计数器
    // *(volatile uint32_t *)(SYS_SP804_BASE + 0x0C) = 0x1;
}
...
int main(void)
{
    uint32_t size_of_tcb = sizeof(OsControlBlock);//1124
    ...
    uint32_t offset_of_IntNestingCount = offsetof(OsControlBlock, IntNestingCount);//1116
    uint32_t offset_of_preempt_flag = offsetof(OsControlBlock, preempt_flag);//1120
